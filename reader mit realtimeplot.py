...
class Reader(tk.Frame):
	def __init__(self, master=None):
		...
		self.checkplot = tk.Checkbutton(f, text='Liveplot', var=plotstat)
		self.checkplot.pack(side='left')
	...
	def reader(self):
                ...
                with serial.Serial(self.dev.get(), 115200, timeout=1) as s:
                        while self.recording:
                                try:
                                        ...
                                        val = float(val)
                                        t = int(t)
                                        val_in_N = (val/91*1000*9,81) # -91 == 1kg load
        ...
        #                        if plotstat.get() == True:
        #                        ani = animation.FuncAnimation(fig, animate, fargs=(t, val_in_N), interval=1000)
        #                        plt.show()
        #                        else:
        #                                continue
